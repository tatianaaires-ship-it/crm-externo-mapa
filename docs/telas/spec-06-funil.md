...
> ✅ **As colunas vazias foram resolvidas (28/07).** O snapshot segue sem trazer tarefa, mas `useRealData()` passou a **simular atividades sobre os pins reais** ([[spec-07-atividades]] §5.5) — então `Visita planejada`, `TD encontrado`, `Perdido` e `Desqualificado` deixam de nascer desertas. **`Aquisição` continua vazia com dado real, e isso é correto:** exige saber se houve pedido, e o `salesforce.lead` não tem essa fonte. A régua do snapshot prevalece em `csc`/`aquisicao` e em pin sem tarefa.
>
