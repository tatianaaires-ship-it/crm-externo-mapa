...
- **O `tipo` da visita não é escolhido em lugar nenhum** (03/08). Ele é **derivado** do estado comercial do ponto ([[tarefa]] §4c) e aparece como **leitura** no sheet de conclusão (§3 item 0), onde também **escolhe o formulário** (§3.2). *(Até 28/07 vivia no sheet de conclusão como chip; até então, no sheet de agendar.)*
...
| Campo | Obrig. | Nota |
|---|---|---|
| ~~**Tipo da visita**~~ | — | ⛔ **Saiu em 03/08.** Eram 3 chips pré-marcados; o tipo virou **derivado** em 5 valores ([[tarefa]] §4c) e **não se escolhe**. O sheet não o mostra nem como leitura: no agendamento o tipo é o de **hoje**, e o que vale é o do dia da visita (§4c.3) — exibir aqui prometeria um propósito que pode mudar até lá |
...
>
> ⚖️ **Num ponto `perdido`/`desqualificado` a nota muda, porque o efeito é outro** (30/07): *"Este ponto está **Desqualificado** e **continua** — agendar não o tira de lá. Ele volta ao funil quando esta visita for **concluída**."* E o toast confirma o mesmo: `Agendada para 12/08 — o pin volta ao funil quando ela for concluída`. Voltar de uma lateral **exige tarefa concluída** ([[tarefa]] §5) — um plano não constata nada —, então a promessa genérica de "coloca no funil" seria mentira justo no caso em que o vendedor está tentando ressuscitar o ponto. ⚠️ **Isto acompanha uma correção de store:** agendar num lateral tirava o pin de lá **pela metade** (ver §9).
...
> 🚪 **Desde 30/07 há DUAS portas para agendar, e esta é a de planejar do zero.** A outra é o **check-out** (§3, item 4), que marca a volta **no instante em que o vendedor sabe qual ela é** — em vez de fechar a visita, sair do sheet e reabrir o pin para agendar. Mesmos campos, mesma escrita (`agendarTarefa`); desde 03/08 o que muda é **só** o `default` de **dia** (aqui **hoje**, lá **vazio** — no check-out o vendedor está no ponto agora, então "hoje" seria sugestão errada). *(A segunda diferença era o **tipo sugerido**, que lá tinha regra própria por olhar a visita que acabou de acontecer; com o tipo derivado ela morreu — §3 item 4.)*

### 2.3 Check-in em qualquer pin, com o tipo já determinado (CAP-6 revisada)
...
**O check-in é um toque só, e o tipo não se decide — ele já está decidido.** ⚠️ **Reescrito em 03/08.** A tarefa nascia com o tipo **sugerido** pelo histórico (`CRM_DATA.sugereTipoVisita`: *nunca visitado → 1ª visita · cliente → Recorrência · visitado e ainda não cliente → Follow-up*) e o vendedor **confirmava ou corrigia** no sheet de conclusão. Agora o tipo é **derivado** do estado comercial do ponto, em cinco valores, e a escada de três da sugestão morreu — ela punha `csc` e cliente recorrente no mesmo `recorrencia`, que é justamente onde os cinco tipos precisam cortar ([[tarefa]] §4c.2). O check-in continua sendo **um toque**; o que mudou é que não há confirmação esperando no fim, só a **leitura** do tipo (§3 item 0).
...
>
> ✅ **A exceção fechou.** Este parágrafo dizia *"sugestão ≠ classificação derivada — `tipo` segue sendo campo digitado, e isso não fere 'classificação nunca é digitada'"*. A ressalva existia porque `tipo` era o **único** campo classificatório digitado do produto; desde 03/08 ele é derivado e **entra na regra** com `qualidade`/`porte`/`origem_confianca`/`status`/`cadastrado`/`status_cliente` ([[estabelecimento]] §8).
>
> **A janela de edição não existe mais:** não há o que editar em nenhum estado. O que o check-out faz é **congelar** o derivado ([[tarefa]] §4c.3) — enquanto a tarefa é `planejada` o tipo acompanha o ponto, e a partir do check-out ela é **registro, não formulário** (§9).
...
0. **Tipo da visita** — **linha de leitura, não campo** (03/08). O tipo é **derivado** do estado comercial do ponto, em **sete** valores, em duas bandas ([[tarefa]] §4c), e vem com o **porquê** ao lado: `♻️ Reaquisição · última compra em 02/04`. O vendedor **vê** e segue; não há chip para tocar. **É este valor que escolhe o formulário** — cada tipo tem o seu (§3.2).

> 🔄 **Isto substitui os chips de tipo, e é o mesmo movimento de 28/07 levado até o fim.** Naquele dia o tipo saiu do check-in e foi para cá com o argumento de que *"quem entra na porta do cliente não para para classificar a visita"* — e a classificação foi empurrada para o último instante possível em vez de sair. Em 03/08 ela **sai**: quem sabe que visita é essa não é o vendedor, é a **base**. Decisão Tatiana: o tipo é pré-determinado pelo status do estabelecimento. Mesmo princípio que tirou o chip de `resultado` (§9) — perguntar o **fato**, nunca a **categoria**.
> ⚖️ **Mostrar o porquê não é enfeite, é o que separa informação de ordem.** Rótulo que o vendedor não pode mudar e não pode entender é a tela mandando: se ele lê `Expansão` num ponto que ele sabe que parou de comprar, precisa ver **de onde saiu** o rótulo para saber que o dado está velho — e a via de correção (com autor e data) é **Fase 4**, declarada em [[tarefa]] §5. É o preço de tirar o campo, e ele está registrado.
...
4. **Próxima visita** (opcional) — um checkbox `Agendar próxima visita` que, ligado, **revela** o mini-form de agendar: **dia** (obrigatório, hoje ou depois) · **hora** (opcional) · **anotação** (opcional). Concluir passa a ser **dois atos num toque**: a atividade fecha e a próxima nasce **planejada**, na Agenda. ⛔ **Os chips de tipo saíram em 03/08**, junto com os do sheet de agendar — o mini-form ficou com três campos.
...
> ⛔ **A heurística do "tipo da próxima" morreu com o campo** (03/08). Havia uma regra própria aqui: `D.sugereTipoVisita` olha o histórico **realizado** e, num ponto nunca visitado, diria "1ª visita" outra vez — então o mini-form usava uma segunda escada (*depois de ir, o que vem é follow-up; em cliente, recorrência*). Com o tipo **derivado na hora de mostrar** ([[tarefa]] §4c.3), não há nada a adivinhar: a tarefa nasce sem tipo gravado e ganha o dela **no dia em que for feita** — inclusive se o pedido chegar no meio. **Duas heurísticas a menos e nenhuma tela pior.**
...
### 3.2 Um formulário por tipo de visita (03/08) — decidido, conteúdo a detalhar

**O check-out deixa de ser um formulário só.** São **sete**, um por tipo ([[tarefa]] §4c.1) — 🚩 1ª visita · 🔁 Follow-up · 🗓️ Recorrência · 📈 Expansão · ♻️ Reaquisição —, porque as cinco visitas fazem perguntas diferentes: *"tem interesse?"* não é *"por que não recomprou?"* não é *"o que mais posso te vender?"*. **Quem escolhe o formulário é o `tipo` derivado**, não o vendedor: ele já está em campo, o app já sabe qual visita é essa, e a tela abre no formulário certo.

**O que fica travado agora, antes de detalhar as perguntas:**

| | Regra |
|---|---|
| ⛔ **Tronco comum, igual nos sete** | os quatro checkboxes de desfecho e o `resultado` derivado deles (§3 item 1) · **motivo obrigatório** em toda realizada sem venda (item 2) · `notas` (item 3) · **agendar a próxima** (item 4) · o padrão `.sform-*` e o **botão que diz o que falta** ([[spec-00-design-system]] §6.7.2) |
| ✅ **O que varia** | as perguntas próprias do propósito de cada tipo — **a detalhar na próxima fatia** |
| ⚠️ **Possível, e é mudança de contrato** | recortar os **14 motivos de não venda** por tipo (`ruptura_de_produto` não faz sentido em 1ª visita). Se for por aí, muda [[tarefa]] §4 e o seed, não só a tela |

> ⚖️ **Por que o tronco não pode variar.** A gerencial (§5) tem **as mesmas colunas para os sete tipos**: a pivô cruza `vendedor × tipo`, a barra empilhada é por `resultado` e a tabela tem uma coluna de `motivo` só. Se cada formulário reinventar o desfecho, o `resultado` deixa de ser comparável entre tipos e os agregados perdem o sentido — sete formulários com sete desfechos são sete relatórios, não um.
>
> ⚠️ **O custo, dito antes de construir:** o check-out é a tela mais usada do app e passa de uma superfície para sete. O que paga é a pergunta certa na visita certa; o que segura o custo é o tronco comum. **Se um tipo não tiver pergunta própria nenhuma, ele não precisa de formulário próprio** — herda o tronco e pronto. Vale decidir isso tipo por tipo na próxima fatia, em vez de nascer com sete arquivos por simetria.
...
| **Tipo de visita** | só a **Gerencial** | `tipo` da tarefa — **7 valores em 2 bandas** desde 03/08 ([[tarefa]] §4c), na ordem fixa do enum. ⚠️ Filtra o **tipo congelado** da realizada, então trocar a regra de derivação **não** reescreve o retrato do passado (§4c.3) |
...
- **O tipo não é escolhido — é derivado, e são CINCO** (03/08, §3 item 0 + [[tarefa]] §4c). O vendedor **vê** o tipo no check-out, com o porquê ao lado, e o tipo **escolhe o formulário** (§3.2). Os chips saíram das **três** telas que os tinham: o sheet de agendar (§2.1), o check-out e o mini-form de próxima visita (§3 item 4) — e com eles morreram duas heurísticas de sugestão. **Alternativa recusada: manter o chip como "correção opcional" do derivado** — campo que existe só para discordar da base reabre a digitação pela porta de trás, e classificação neste produto nunca é digitada ([[estabelecimento]] §8). **Preço aceito e declarado:** dado comercial atrasado no ERP dá formulário errado **sem escape em campo**; a mitigação de hoje é mostrar a razão da classificação, e a via de correção com autor e data é **Fase 4**.
- **`recorrencia` encolheu de significado** (03/08). Era *"o pin é cliente"* (`csc`/`aquisicao`); virou *"comprou 1 ou 2 vezes, o ciclo de recompra não fechou"* — e quem já é recorrente virou **`expansao`**. Consequência de demo: a pivô estrearia com duas colunas vazias sobre o estado v9, então **o seed precisa resemear** ([[tarefa]] §9), pelo mesmo motivo que subiu o v7 em 28/07.
- **O tipo era confirmado no fim, não no começo** (§2.3 + §3) — ⚠️ **superado em 03/08 pela linha acima:** agora não é confirmado, é **exibido**. O registro de 28/07 fica porque é o mesmo princípio andando na mesma direção: naquele dia a classificação foi empurrada para o último instante em que a informação existia; em 03/08 se descobriu que ela **nunca** foi informação do vendedor. Ele esteve em chips no pin, acima do botão de check-in, por uma hora em 28/07: quem entra na porta do cliente não para para classificar a visita — e no check-in ainda não sabe o que ela vai ser. Foi para o sheet de conclusão, onde a informação existe. Alternativa recusada: um passo de confirmação **antes** do check-in (sheet ou prompt) — um toque a mais na ação mais frequente do app.
...
- **Agendar num pin `perdido`/`desqualificado` tirava o pin da lateral pela METADE** — bug encontrado em 30/07 ao abrir a porta de agendamento no check-out, e corrigido no store. Voltar de uma lateral é *restaurar a etapa de origem e então aplicar o status novo*; a restauração era escrita **antes** de a recusa ser decidida, então `visita_planejada` (que é regressão na escada, logo recusado) deixava o pin em *Visitado*, sem `status_anterior`, com o `motivo_status` da perda colado, e a função dizendo que nada mudou — nem `status_cliente` era rederivado. Três consertos: a transição virou **atômica**, sair da lateral **apaga `motivo_status`**, e `agendarTarefa` **nunca** tira o pin de uma lateral (guarda explícita, porque um lateral sem `status_anterior` — snapshot, dado antigo — cairia em `sem_plano`, de onde `visita_planejada` **é** avanço legítimo). O sheet e o toast passaram a dizer que o ponto continua onde está (§2.1). Ver [[estabelecimento]] §5 e [[spec-06-funil]] §4. ⚖️ **O padrão: transição recusada não escreve nada** — "mudou pela metade e devolveu `false`" é pior que recusar, porque some do log e aparece no pin.
