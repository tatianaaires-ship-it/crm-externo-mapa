...
| **Vendedor** | barras horizontais proporcionais | Comparação entre categorias nomeadas. Uma cor só para todas as barras (`--brand`) — colorir por tamanho seria duplicar o comprimento numa segunda variável |
| **Vendedor × tipo** | **tabela pivô** com totais de linha e coluna | Cruzamento de duas dimensões nominais — nem barra nem gráfico resolve; a grade é a forma certa. Números à direita e com `tabular-nums` (aqui dígito de largura fixa **ajuda**, ao contrário do texto corrido). **Cada célula é um drill duplo** (vendedor **e** tipo). Substituiu a quebra "Por tipo" em chips, que a pivô contém |

- **Cada número filtra a tabela do §5.4** e rola até ela — é o requisito explícito do Notion: *quantidade **+** lista detalhada*. Vale para os três tiles, cada célula da pivô (drill **duplo**: vendedor **e** tipo), cada linha de vendedor e cada item de legenda. Um **chip no título da tabela** diz o critério ativo e é a porta de volta — tabela filtrada sem chip parece dado que sumiu. Trocar qualquer filtro da barra **limpa o drill** (ele pode ser justamente pela dimensão que mudou).
...
Não repete tokens nem componentes: reusa o pin-sheet (§6.7), o sheet/painel bottom (§6.6), o modal de criação (§6.8), o card de lead como molde do card de atividade (§6.9), os transientes (§6.14), o shell/nav (§5) e o **chip** (§6.2) nos presets de período. **Pede ao SPEC 00:** a 4ª aba na navegação (§5.2), **três cores novas** de status (`csc`, `perdido`, `desqualificado`) e um badge de `resultado` (4 valores) — ambas em §2.6 — mais **quatro componentes novos** registrados agora: o **segmented control** (§6.10), o **pill de filtro herdado** (§6.11), a **barra de filtros de aba** (§6.12) e os **gráficos da gerencial** (§6.13, com as regras de cor que passam a valer para qualquer gráfico do produto).
