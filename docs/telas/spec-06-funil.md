...
> 🔴 **REVERTIDO em 06/08 — a aba Funil passa a ter DUAS abas, e a piscina ENTRA no board.** Decisões de Tatiana, detalhadas em [[jornada-funil-aquisicao]] §5.1 e §5.2. ⚠️ **Esta spec inteira ainda descreve o board antigo**; até ela ser reescrita, o que vale é a jornada.
> - **Segmented control com dois boards:** **Aquisição** — *Perdido · Prospecção · Qualificação · Fechamento · Aquisição · Desqualificado* — e **Recorrência** — *Perdido · Disponível 2ª · Disponível 3ª · Recorrente*. O primeiro é **esteira** (avança pelo trabalho); o segundo é **retrato** (muda pelo ERP) e nasce **sem arraste**. ⭐ As colunas da Recorrência vêm do **`Status_funil__c`, que a operação já mantém** ([[fluxos-n8n-salesforce]] §3e) — não são os tipos de visita, e **o campo se apaga sozinho aos 45 dias**, então a coluna **não pode ser cacheada**. Em ambos, **o estado de risco abre à esquerda**.
> - **A coluna `Aquisição` mostra os ganhos do MÊS** e **zera no dia 1º** — é a única com recorte de tempo, e isso é pedido, não bug.
> - **Some 4 colunas:** *Visita planejada* e *Visitado* viram **tags** dentro de Prospecção (junto com a **piscina `sem_plano`**), *TD encontrado* vira **Qualificação**, e **CSC sai do funil** — vira `status_cliente`.
> - **O card muda** (§7 desta spec): o **badge de qualidade sai** e entram **duas tags** — *estágio* (só em Prospecção) e ***`status_cliente`*** (em qualquer coluna). É a tag `churn` que faz a **visita de reaquisição** aparecer no board.
> - **O arraste encolhe:** Qualificação e Fechamento passam a **recusar drop** pela mesma razão de CSC/Aquisição — são constatação de campo. Sobram Perdido e Desqualificado, que já exigem motivo. O board de aquisição fica **quase todo somente-leitura**, coerente com *"`status` nunca é digitado"*.
> - **Agendar deixa de pôr o pin no board** (ele já está na piscina): só troca a tag `oportunidade` → `planejada`. Morre *"cancelar a última planejada tira o pin do board"* — a reversão continua, sem sair da tela.
> - 📏 **O número que a decisão carrega:** com dado real a coluna Prospecção nasce com **milhares** de cards contra dezenas nas outras; a mitigação declarada é **tag + filtro**, e a saída da qualidade do card tira o único canal de priorização que existia ali.

- ⚠️ **`sem_plano` é o 8º valor do enum e NÃO tem coluna.** *(Vale até 06/08 — ver o bloco acima.)* O funil é o **pipeline de trabalho, não a base** ([[estabelecimento]] §5): o pin entra no board quando ganha uma **visita planejada** (tarefa agendada) e sai dele se o plano for cancelado. Pin sem plano continua no **mapa** e na **Inteligência**.
