...
> ⚖️ **O `tipo` de cada parada não é escolhido nem gravado — é DERIVADO na hora da visita** ([[tarefa]] §4c). O modo não pergunta nada disso, e desde 03/08 nem precisa da ressalva: cada parada ganha o seu tipo, entre os **cinco**, no dia em que for feita, a partir do estado comercial do ponto. *(Era `D.sugereTipoVisita` por pin — sugestão que o vendedor confirmava no check-out. O argumento original segue valendo e ficou mais forte: pedir o tipo de N pontos numa tela de seleção espacial seriam N escolhas para um gesto que é **um só**.)* ✅ **Ganho de graça:** uma rota montada hoje para a semana que vem não fossiliza propósito nenhum — se o pedido de um dos pontos entrar antes da visita, aquela parada muda de tipo sozinha ([[tarefa]] §4c.3).
